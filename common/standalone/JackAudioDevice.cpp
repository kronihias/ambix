...
// Vendored from JUCE's juce_JackAudio.cpp (juce_audio_devices). Built as its own
// translation unit (not part of the JUCE module unity build) so the app owns the
// JACK backend without patching JUCE. Differences vs upstream: ports are sized
// to the plug-in's channel count and opened in open(), no auto-connect of in or
// out, all ports kept active, and the client reports its own ports. Registered
// with the standalone's AudioDeviceManager via createJackAudioDeviceType();
// JUCE itself is built with JUCE_JACK=0 so there is no duplicate JACK type.
...
struct JackPortIterator
{
    ...
    {
        if (client != nullptr)
            ports.reset (jsa::jack_get_ports (client, nullptr, nullptr,
    ...
    struct Free
    {
        void operator() (const char** ptr) const noexcept { jsa::jack_free (ptr); }
    };
...
public:
          ...
          notifyChannelsChanged (std::move (notifyIn))
    {
        ...
        client = jsa::jack_client_open (JUCE_JACK_CLIENT_NAME, JackNoStartServer, &status);
        ...
        {
            jsa::jack_set_error_function (errorCallback);

            // Ports are registered in open() based on the plugin's channel count,
            // not on the selected external client. This decouples the number of
            // exposed ports from whatever JACK device is picked in the settings.
        }
    }
    ...
    // report this client's own ports rather than the selected external client's,
    // so the settings dialog reflects the ports actually exposed to JACK
    StringArray getOutputChannelNames() override
    {
        StringArray names;
        for (int i = 1; i <= totalNumberOfOutputChannels; ++i)
            names.add ("out_" + String (i));
        return names;
    }
    ...
    {
        StringArray names;
        for (int i = 1; i <= totalNumberOfInputChannels; ++i)
            names.add ("in_" + String (i));
        return names;
    }
    ...
    Array<double> getAvailableSampleRates() override
    {
        ...
        if (client != nullptr)
            rates.add (jsa::jack_get_sample_rate (client));
    ...
    Array<int> getAvailableBufferSizes() override
    {
        ...
        if (client != nullptr)
            sizes.add (static_cast<int> (jsa::jack_get_buffer_size (client)));

        return sizes;
    }

    int getDefaultBufferSize() override             { return getCurrentBufferSizeSamples(); }
    int getCurrentBufferSizeSamples() override      { return client != nullptr ? static_cast<int> (jsa::jack_get_buffer_size (client)) : 0; }
    double getCurrentSampleRate() override          { return client != nullptr ? static_cast<int> (jsa::jack_get_sample_rate (client)) : 0; }
                 ...
                 double /* sampleRate */, int /* bufferSizeSamples */) override
    {
        ...
        for (int i = 0; i < inputChannels.getHighestBit()+1; ++i)
        {
            ...
            inputPortName << "in_" << ++totalNumberOfInputChannels;

            inputPorts.add (jsa::jack_port_register (client, inputPortName.toUTF8(),
        ...
        for (int i = 0; i < outputChannels.getHighestBit()+1; ++i)
        {
            ...
            outputPortName << "out_" << ++totalNumberOfOutputChannels;

            outputPorts.add (jsa::jack_port_register (client, outputPortName.toUTF8(),
        ...
        jsa::jack_set_process_callback (client, processCallback, this);
        jsa::jack_set_port_connect_callback (client, portConnectCallback, this);
        jsa::jack_on_shutdown (client, shutdownCallback, this);
        jsa::jack_on_info_shutdown (client, infoShutdownCallback, this);
        jsa::jack_set_xrun_callback (client, xrunCallback, this);
        jsa::jack_activate (client);
        deviceIsOpen = true;

        // do not auto connect inputs
        /*
        if (! inputChannels.isZero())
        {
            forEachClientChannel (inputName, false, [&] (const char* portName, int index)
            {
                ...
                jassert (jsa::jack_port_flags (jsa::jack_port_by_name (client, source)) & JackPortIsOutput);
                jassert (jsa::jack_port_flags (inputPort) & JackPortIsInput);

                auto error = jsa::jack_connect (client, source, jsa::jack_port_name (inputPort));
                if (error != 0)
        ...
        */
        ...
        if (! outputChannels.isZero())
        {
            forEachClientChannel (outputName, true, [&] (const char* portName, int index)
            {
                ...
                jassert (jsa::jack_port_flags (outputPort) & JackPortIsOutput);
                jassert (jsa::jack_port_flags (jsa::jack_port_by_name (client, destination)) & JackPortIsInput);

                auto error = jsa::jack_connect (client, jsa::jack_port_name (outputPort), destination);
                if (error != 0)
    ...
    void close() override
    {
        ...
        if (client != nullptr)
        {
            [[maybe_unused]] const auto result = jsa::jack_deactivate (client);
            jassert (result == 0);

            jsa::jack_set_xrun_callback (client, xrunCallback, nullptr);
            jsa::jack_set_process_callback (client, processCallback, nullptr);
            jsa::jack_set_port_connect_callback (client, portConnectCallback, nullptr);
            jsa::jack_on_shutdown (client, shutdownCallback, nullptr);
            jsa::jack_on_info_shutdown (client, infoShutdownCallback, nullptr);

            // unregister ports so a re-open (e.g. sample-rate change) starts clean
            // instead of accumulating extra ports on top of the existing ones
            for (auto* port : inputPorts)
                jsa::jack_port_unregister (client, port);
    ...
    int getOutputLatencyInSamples() override
    {
        ...
        for (int i = 0; i < outputPorts.size(); i++)
            latency = jmax (latency, (int) jsa::jack_port_get_total_latency (client, outputPorts[i]));
    ...
    int getInputLatencyInSamples() override
    {
        ...
        for (int i = 0; i < inputPorts.size(); i++)
            latency = jmax (latency, (int) jsa::jack_port_get_total_latency (client, inputPorts[i]));
    ...
    void process (const int numSamples)
    {
        ...
        for (int i = 0; i < totalNumberOfInputChannels; ++i)
        {
            if (activeInputChannels[i])
                if (auto* in = (jack_default_audio_sample_t*) jsa::jack_port_get_buffer (inputPorts.getUnchecked (i),
        ...
        for (int i = 0; i < totalNumberOfOutputChannels; ++i)
        {
            if (activeOutputChannels[i])
                if (auto* out = (jack_default_audio_sample_t*) jsa::jack_port_get_buffer (outputPorts.getUnchecked (i),
                                                                                           static_cast<jack_nframes_t> (numSamples)))
...
public:
    ...
    void scanForDevices()
    {
        ...
        // open a dummy client
        if (auto* const client = jsa::jack_client_open ("JuceJackDummy", JackNoStartServer, &status))
        {
...
{
    return new jsa::JackAudioIODeviceType();
}
